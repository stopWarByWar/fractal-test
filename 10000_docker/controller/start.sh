...
time python test.py exec -c 'bash copy.sh test.toml'


time python test.py exec -c 'bash run_docker.sh 14'
...
time python test.py exec -c "bash set_docker_hostname.sh"

killall -9 gftl
rm -rf data/chaindata
nohup ./gftl --unlock 123 --config test.toml --genesisAlloc genesis_alloc.json --rpc --rpcport 8545 --datadir ./data --port 30503 --pprof --pprofport 6060 --verbosity 3 > ./test.log 2>&1 &

time python test.py exec -c 'cd /root/run && bash startPacker.sh 10'
sleep 3

#time python test.py docker -c 'cd /root/docker_run && bash startNode.sh'
time python test.py exec -c "bash start_docker_node.sh"

time python test.py exec -c 'cd /root/run && bash startTx.sh'
